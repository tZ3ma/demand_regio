...
def get_config(filename=None, use_ruamel=False, **kwargs):
    ...
    if use_ruamel:
        ...
        import _ruamel_yaml as ruamel_yaml
    else:
...
def database_raw(query, force_update=False):
    ...
    if os.path.exists(filename) and (force_update is False):
        ...
        try:
            df = pd.read_json(requests.get(host + query).content.decode("utf-8"),
                              dtype=get_config()['dtypes'])
        except ValueError:
            ser = pd.read_json(requests.get(host + query).content.decode("utf-8"),
